 ...
 const  Home =  () => {
  ...
  const [apiValue, setApiValue] = useState([]);
   useEffect( () => {
    ...
    debugger;
     axios(`https://cryptocurrencies.ventures/api/v1/markets`)
            .then(response => response.data)
            .then((data) => {
                console.log(data);
                  setApiValue(data.slice(0,4));
           ...
           <div className="card overflow-hidden">
             <div className="card-header align-items-start border-0 pb-0">
               <div className="mr-auto">
                 ...
                 <h2 className="text-black mb-0 font-w600">${item.high_24h}</h2>
               </div>
               <img  width={42} height={42} src= {item.image}/>
               <svg
                 width={42}
                 height={42}
                 viewBox="0 0 42 42"
